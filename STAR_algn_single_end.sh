...
tail -n+2 $sampfile | while read line; do
	...
	command="STAR --runThreadN 24 --genomeDir /ebio/ecnv_projects/common_resourses/data/reference_genome/GRCz11/ --readFilesIn $sample.fastq.gz --readFilesCommand gunzip -c --sjdbInsertSave All --quantMode GeneCounts --outReadsUnmapped Fastq --outSAMstrandField intronMotif --outSAMtype BAM SortedByCoordinate --outFileNamePrefix STAR_output/$sample --genomeLoad LoadAndKeep --limitBAMsortRAM 20000000000 $optional"
